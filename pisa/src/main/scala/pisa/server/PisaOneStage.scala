...
//  println("Initialisation finished.")
  ...
  def getFacts(top_level_state: ToplevelState, state_string: String) : String = {
    var facts : String = ""
    if (state_string.trim.nonEmpty) {
      // Limit the maximum number of local facts to be 10
      for (fact <- make_pretty_list_string_list(pretty_local_facts(top_level_state, false)).retrieveNow.takeRight(10)) {
  ...
  def readImports(): String = {
    var source = TheoryManager.Text(getStarterString.trim.replaceAll("\n", " ").trim, setup.workingDirectory.resolve(""))
    val header = getHeader(theoryStarter)
//    The master directory, currently is set to the source path instead of the parent of the source path
//    For the afp extraction experiments, the parent path was used
//    val masterDir = source.path.getParent
    val masterDir = source.path
  ...
  def isabelleCommand(isa_command: IsaCommand): ZIO[
    ...
    if (isa_command.command.startsWith("<get state>")) {
      ...
      if (pisaos.top_level_state_map.contains(tls_name)) {
        ...
        proof_state = s"${pisaos.getStateString(pisaos.retrieve_tls(new_name))}"
      }
      else proof_state = "Didn't find top level state of given name"
    }
    ...
    } else if (isa_command.command.trim.startsWith("<get_facts>")){
      val tls_name : String = isa_command.command.trim.stripPrefix("<get_facts>").trim
      val top_level_state : ToplevelState = pisaos.retrieve_tls(tls_name: String)
      val state_string : String = pisaos.getStateString(top_level_state)
      proof_state = pisaos.getFacts(top_level_state, state_string)
    } else if (isa_command.command.trim.startsWith("<get_imports>")){
      proof_state = pisaos.readImports()
    } else if (isa_command.command.trim.startsWith("<get_proof_level>")){
      val tls_name : String = isa_command.command.trim.stripPrefix("<get_proof_level>").trim
      val top_level_state : ToplevelState = pisaos.retrieve_tls(tls_name: String)
      proof_state = pisaos.getProofLevel(top_level_state: ToplevelState).toString
    }
     else {
      proof_state = pisaos.step(isa_command.command)
    }
